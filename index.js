...
async function run() {
  try {
    ...
    app.get('/courses',async(req,res)=>{
        ...
        const result = await courseColluction.find(catObj).sort(sortObj).limit(limit).toArray()
        res.send(result)
    })
    ...
    app.get('/course-search',async(req,res)=>{
      ...
      const result = await courseColluction.find({ "contestName":{$regex: ".*"+search+".*",$options:'i' }}).toArray();
      res.send(result)
    })
